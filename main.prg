...
local oItem
    ...
    oTopBar:ColorSpec := SI_MENU
            ...
            oTopBar:AddItem( MenuItem ( '&Sistema', oSistema ) )
                ...
                oItem := MenuItem( 'A&postadores   Ctr-P' ,{|| Apostadores() },  K_CTRL_P, 'Realiza a manutencao de Apostadores')
                oSistema:AddItem( oItem )

                oItem := MenuItem( '&Apostas       Ctr-A' ,{|| MntAposta() },    K_CTRL_A, 'Realiza a manutencao do cadastro de Apostas')
                oSistema:AddItem( oItem )                
...
// Edilson
            ...
            If SI_CONCURSO == 'LC' .or. SI_CONCURSO == 'LG'
                ...
                oItem := MenuItem( 'Clube&s         Ctr-S' ,{|| Clubes() },    K_CTRL_S, 'Realiza a emissao das movimentacoes dos clubes.')
                oRelato:AddItem( oItem )
...
*
*	Retorna a cor utilizada nos campos de Browse.
*
*/
...
FUNCTION SysFieldGRadioBox()
    ...
    /***
*
*	SiFieldCheckBox() --> String
*
*	Retorna a cor utilizada nos campos de CheckBox.
*
*/
FUNCTION SysFieldCheckBox
    return( SI_FIELDCHECKBOX )
...
/***
*
*  od_Random( <nLimite> ) --> nResult
*
*	 Gera um numero inteiro aleatorio que deve estar entre 1 e o valor definido no paramentro limite.
*
*  <nLimite> : E o valor limite no qual o valor deve ser gerado.
*
*  nResult   : Retorna um valor inteiro numerico aleatorio.
*
*/
FUNCTION od_Random( nLimite )
...
local nRetValue
    ...
    DEFAULT nLimite TO 100
    
    while nLimite > nCalc
        nCalc := nCalc * 100 + Seconds()
    enddo
    
    nGuarda   := ( nGuarda + nCalc ) / ( nRetValue := nGuarda * nCalc % nLimite +1 )
    nRetValue := Int( nRetValue )
...
STATIC PROCEDURE SetupEnvironment()
    ...
    ErrorTable( '003', 'Codigo ja exitente.' )
...
INIT PROCEDURE InitSystem()
    ...
    SI_DBCODEPAGE    := Set( _SET_DBCODEPAGE, hb_CdpSelect( 'PT850' ) )
    
    SI_DBFLOCKSCHEME := Set( _SET_DBFLOCKSCHEME, DB_DBFLOCK_DEFAULT )
...
procedure importar()
	...
	while .not. TMP->( Eof() )
		...
		APOSTADORES->APO_CODIGO  := PadL( Right( AllTrim( TMP->APO_CODIGO ), 5 ), 5, '0' )
	...
	while .not. TMP->( Eof() )
		...
		APOSTAS->( dbAppend() )
		APOSTAS->APT_JOGO   := cJogo
		APOSTAS->APT_CONCUR := PadL( AllTrim( TMP->AP1_CONCUR ), 5, '0' )
		APOSTAS->APT_SEQUEN := '001'
		APOSTAS->APT_SORTEI := TMP->AP1_SORTEI
		APOSTAS->APT_ORIG   := 'SIS'
		APOSTAS->APT_HISTOR := 'APOSTA NORMAL'
		APOSTAS->( dbUnlock() )
	...
	while .not. TMP->( Eof() )
		...
		while TMP->AP2_JOGO+TMP->AP2_CONCUR == cComp
			...
			APOSTAS_ITENS->ITN_CONCUR := PadL( AllTrim( TMP->AP2_CONCUR ), 5, '0' )
			APOSTAS_ITENS->ITN_SEQUEN := '001'			
			APOSTAS_ITENS->ITN_ITEM   := StrZero( nSequencia++, 3 )
	...
	while .not. TMP->( Eof() )
		...
		while TMP->AP3_JOGO+TMP->AP3_CONCUR == cComp
			...
			while TMP->AP3_JOGO+TMP->AP3_CONCUR+TMP->AP3_DIGITO == cComp2
				...
				APOSTAS_CLUBES->CLB_CONCUR := PadL( AllTrim( TMP->AP3_CONCUR ), 5, '0' )
				APOSTAS_CLUBES->CLB_SEQUEN := '001'
				APOSTAS_CLUBES->CLB_ITEM   := StrZero( nSequencia, 3 )				
				APOSTAS_CLUBES->CLB_FAIXA  := TMP->AP3_FAIXA
				APOSTAS_CLUBES->CLB_COL1   := PadL( Right( AllTrim( TMP->AP3_COL1 ), 5 ), 5, '0' )
				APOSTAS_CLUBES->CLB_COL2   := PadL( Right( AllTrim( TMP->AP3_COL2 ), 5 ), 5, '0' )
	...
	while .not. TMP->( Eof() )
		...
		APOSTAS_GRUPOS->GRP_CONCUR := PadL( AllTrim( TMP->GRP_CONCUR ), 5, '0' )
		APOSTAS_GRUPOS->GRP_SEQUEN := '001'
		APOSTAS_GRUPOS->GRP_APOCOD := PadL( Right( AllTrim( TMP->GRP_APOSTA ), 5 ), 5, '0' )
