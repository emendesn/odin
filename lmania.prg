	...
	If SystemConcurso() == pLOTO_MANIA
		...
		If ( nPointer := AScan( pSTRU_SYSTEM, { |xJog| xJog[ pSTRU_JOGO ] == pLOTO_MANIA } ) ) > 0
			...
			begin sequence
				...
				bFiltro := { || CONCURSO->CON_JOGO == pLOTO_MANIA .and. .not. CONCURSO->( Eof() ) }
		...
		If ( nPointer := AScan( pSTRU_SYSTEM, { |xJog| xJog[ pSTRU_JOGO ] == pLOTO_MANIA } ) ) > 0
			...
			begin sequence
				...
				If ( cAutoSequence := oIniFile:ReadString( 'LOTOMANIA', 'AUTO_SEQUENCE', '0' ) ) == '1'
					...
					dbEval( { || nCodigo++ }, { || CONCURSO->CON_JOGO == pLOTO_MANIA .and. .not. CONCURSO->( Eof() ) } )
...
local bFiltro      := { || CONCURSO->CON_JOGO == pLOTO_MANIA .and. .not. CONCURSO->( Eof() ) }
local oBarProgress
local oPDFReport
local nLinha
			...
			while lContinua
				...
				If lContinua .and. LastKey() != K_ESC
					...
					If CONCURSO->( dbSetOrder(1), dbSeek( pLOTO_MANIA + cInicio ) ) .and. ;
						...
						bFiltro := { || CONCURSO->CON_JOGO == pLOTO_MANIA .and. .not. CONCURSO->( Eof() ) .and. ;
