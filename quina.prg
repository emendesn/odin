...
local nGrade
	...
	If SystemConcurso() == pQUINA

		If ( nPointer := AScan( pSTRU_SYSTEM, { |xJog| xJog[ pSTRU_JOGO ] == pQUINA } ) ) > 0
			...
			begin sequence
				...
				bFiltro := { || CONCURSO->CON_JOGO == pQUINA .and. CONCURSO->( .not. Eof() ) }

				dbSelectArea('CONCURSO')
				CONCURSO->( dbEval( {|| nMaxItens++ }, bFiltro ) )
				CONCURSO->( dbSetOrder(2), dbSeek( pQUINA ) )
...
*	QNAIncluir()
*
*	Realiza a inclusao dos dados para o concurso da QUINA.
*
*   QNAMntBrowse -> QNAIncluir
*
*/
STATIC PROCEDURE QNAIncluir
	...
	If SystemConcurso() == pQUINA
		...
		If ( nPointer := AScan( pSTRU_SYSTEM, { |xJog| xJog[ pSTRU_JOGO ] == pQUINA } ) ) > 0
			...
			begin sequence
				...
				// Inicializa as Variaveis de no vetor aQuina
				xStoreQuina
				...
				//
				If ( cAutoSequence := oIniFile:ReadString( 'QUINA', 'AUTO_SEQUENCE', '0' ) ) == '1'
					// Define o codigo sequencial
					dbEval( { || nCodigo++ }, { || CONCURSO->CON_JOGO == pQUINA .and. CONCURSO->( .not. Eof() ) } )
					pQNA_CONCURSO := StrZero( nCodigo, 5 )
				EndIf
				...
				while lContinua
					...
					@ oWindow:nTop+ 1, oWindow:nLeft+14 GET     pQNA_CONCURSO                                  ;
														PICT    '@K 99999'                                     ;
														SEND    VarPut(StrZero(Val(ATail(GetList):VarGet()),5));
														CAPTION 'Concurso'                                     ;
														COLOR   SysFieldGet()

					@ oWindow:nTop+ 1, oWindow:nLeft+30 GET     pQNA_SORTEIO                                   ;
														PICT    '@KD 99/99/99'                                 ;
														CAPTION 'Sorteio'                                      ;
														COLOR   SysFieldGet()
					...
					@ oWindow:nTop+ 3, oWindow:nLeft+14 GET   pQNA_DEZENA_01                                   ;
														PICT  '@K 99'                                          ;
														SEND  VarPut(StrZero(Val(ATail(GetList):VarGet()),2))  ;
														COLOR SysFieldGet()

					@ oWindow:nTop+ 3, oWindow:nLeft+18 GET   pQNA_DEZENA_02                                   ;
														PICT  '@K 99'                                          ;
														SEND  VarPut(StrZero(Val(ATail(GetList):VarGet()),2))  ;
														COLOR SysFieldGet()

					@ oWindow:nTop+ 3, oWindow:nLeft+22 GET   pQNA_DEZENA_03                                   ;
														PICT  '@K 99'                                          ;
														SEND  VarPut(StrZero(Val(ATail(GetList):VarGet()),2))  ;
														COLOR SysFieldGet()

					@ oWindow:nTop+ 3, oWindow:nLeft+26 GET   pQNA_DEZENA_04                                   ;
														PICT  '@K 99'                                          ;
														SEND  VarPut(StrZero(Val(ATail(GetList):VarGet()),2))  ;
														COLOR SysFieldGet()

					@ oWindow:nTop+ 3, oWindow:nLeft+30 GET   pQNA_DEZENA_05                                   ;
														PICT  '@K 99'                                          ;
														SEND  VarPut(StrZero(Val(ATail(GetList):VarGet()),2))  ;
														COLOR SysFieldGet()
					...
					// Coluna de Acertos			
					@ oWindow:nTop+ 6, oWindow:nLeft+10 GET     pQNA_ACERTO_05                                 ;
														PICT    '@EN 9,999,999,999'                            ;
														CAPTION 'Quina'                                        ;
														COLOR   SysFieldGet()                                  ;
														WHEN    AvalCondRateio( pSTRU_SYSTEM[ nPointer ][ pSTRU_REGRA_PREMIACAO ], '05', pQNA_SORTEIO )

					@ oWindow:nTop+ 7, oWindow:nLeft+10 GET     pQNA_ACERTO_04                                 ;
														PICT    '@EN 9,999,999,999'                            ;
														CAPTION 'Quadra'                                       ;
														COLOR   SysFieldGet()                                  ;
														WHEN    AvalCondRateio( pSTRU_SYSTEM[ nPointer ][ pSTRU_REGRA_PREMIACAO ], '04', pQNA_SORTEIO )

					@ oWindow:nTop+ 8, oWindow:nLeft+10 GET     pQNA_ACERTO_03                                 ;
														PICT    '@EN 9,999,999,999'                            ;
														CAPTION 'Terno'                                        ;
														COLOR   SysFieldGet()                                  ;
														WHEN    AvalCondRateio( pSTRU_SYSTEM[ nPointer ][ pSTRU_REGRA_PREMIACAO ], '03', pQNA_SORTEIO )

					@ oWindow:nTop+ 9, oWindow:nLeft+10 GET     pQNA_ACERTO_02                                 ;
														PICT    '@EN 9,999,999,999'                            ;
														CAPTION 'Duque'                                        ;
														COLOR   SysFieldGet()                                  ;
														WHEN    AvalCondRateio( pSTRU_SYSTEM[ nPointer ][ pSTRU_REGRA_PREMIACAO ], '02', pQNA_SORTEIO )

					// Coluna de Premios			
					@ oWindow:nTop+ 6, oWindow:nLeft+24 GET   pQNA_PREMIO_05                                   ;
														PICT  '@EN 99,999,999,999.99'                          ;
														COLOR SysFieldGet()                                    ;
														WHEN  AvalCondRateio( pSTRU_SYSTEM[ nPointer ][ pSTRU_REGRA_PREMIACAO ], '05', pQNA_SORTEIO )

					@ oWindow:nTop+ 7, oWindow:nLeft+24 GET   pQNA_PREMIO_04                                   ;
														PICT  '@EN 99,999,999,999.99'                          ;
														COLOR SysFieldGet()                                    ;
														WHEN  AvalCondRateio( pSTRU_SYSTEM[ nPointer ][ pSTRU_REGRA_PREMIACAO ], '04', pQNA_SORTEIO )

					@ oWindow:nTop+ 8, oWindow:nLeft+24 GET   pQNA_PREMIO_03                                   ;
														PICT  '@EN 99,999,999,999.99'                          ;
														COLOR SysFieldGet()                                    ;
														WHEN  AvalCondRateio( pSTRU_SYSTEM[ nPointer ][ pSTRU_REGRA_PREMIACAO ], '03', pQNA_SORTEIO )

					@ oWindow:nTop+ 9, oWindow:nLeft+24 GET   pQNA_PREMIO_02                                   ;
														PICT  '@EN 99,999,999,999.99'                          ;
														COLOR SysFieldGet()                                    ;
														WHEN  AvalCondRateio( pSTRU_SYSTEM[ nPointer ][ pSTRU_REGRA_PREMIACAO ], '02', pQNA_SORTEIO )
					...
					@ oWindow:nBottom- 1, oWindow:nRight-22 GET     lPushButton PUSHBUTTON                     ;
															...
															COLOR   SysPushButton()                            ;
															STYLE   ''                                         ;
															WHEN    Val( pQNA_CONCURSO ) > 0 .and.             ;
																	.not. Empty( pQNA_SORTEIO ) .and.          ;
																	.not. Empty( pQNA_DEZENA_01 ) .and.        ;
																	.not. Empty( pQNA_DEZENA_02 ) .and.        ;
																	.not. Empty( pQNA_DEZENA_03 ) .and.        ;
																	.not. Empty( pQNA_DEZENA_04 ) .and.        ;
																	.not. Empty( pQNA_DEZENA_05 )              ;
					...
					If lContinua .and. LastKey() != K_ESC
						...
						pQNA_CONCURSO := StrZero( Val( pQNA_CONCURSO ), 5 )

						//************************************************************************
						//*Verifica se concurso ja existe                                        *
						//************************************************************************
						If .not. CONCURSO->( dbSetOrder(1), dbSeek( pQUINA + pQNA_CONCURSO ) )

							//************************************************************************
							//*Verifica a duplicidade de dezenas no concurso                         *
							//************************************************************************
							If xDuplicSequencia( aQuina[ pQNA_POS_DADOS ][ pQNA_POS_DEZENAS ] )

								//************************************************************************
								//*Verifica cada item da primeira sequencia para identificar se o valor  *
								//*digitado esta dentro da faixa especifica do concurso                  *
								//************************************************************************
								If xVerificaSequencia( 	aQuina[ pQNA_POS_DADOS ][ pQNA_POS_DEZENAS ], ;
			...
			always
				...
				oIniFile:WriteString( 'QUINA', 'AUTO_SEQUENCE', cAutoSequence )
...
*	QNAModificar()
*
*	Realiza a manutencao dos dados para o concurso da QUINA.
*
*   QNAMntBrowse -> QNAModificar
*
*/
STATIC PROCEDURE QNAModificar
...
memvar xCount, xTemp
	...
	//************************************************************************
	// A rotina so deve ser executada a partir de concurso quina
	//************************************************************************
	If CONCURSO->CON_JOGO == pQUINA
		...
		If ( nPointer := AScan( pSTRU_SYSTEM, { |xJog| xJog[ pSTRU_JOGO ] == pQUINA } ) ) > 0
			...
			begin sequence
				...
				//
				pQNA_CONCURSO := CONCURSO->CON_CONCUR
				pQNA_SORTEIO  := CONCURSO->CON_SORTEI
				...
				If JOGOS->( dbSetOrder(1), dbSeek( CONCURSO->CON_JOGO + CONCURSO->CON_CONCUR ) )
					while nPos++ <= Len( AllTrim( JOGOS->JOG_DEZENA ) )
						If IsDigit( SubStr( AllTrim( JOGOS->JOG_DEZENA ), nPos, 1 ) )
							aQuina[ pQNA_POS_DADOS ][ pQNA_POS_DEZENAS ][ nCount++ ] := SubStr( AllTrim( JOGOS->JOG_DEZENA ), nPos++, 2 )
						EndIf
					enddo
				EndIf
				...
				If RATEIO->( dbSetOrder(1), dbSeek( CONCURSO->CON_JOGO + CONCURSO->CON_CONCUR ) )
					while RATEIO->RAT_JOGO == CONCURSO->CON_JOGO .and.  ;
						...
						RATEIO->( Eof() )
						do case
							case RATEIO->RAT_FAIXA == '02'
								pQNA_ACERTO_02 := RATEIO->RAT_ACERTA
								pQNA_PREMIO_02 := RATEIO->RAT_RATEIO
							case RATEIO->RAT_FAIXA == '03'
								pQNA_ACERTO_03 := RATEIO->RAT_ACERTA
								pQNA_PREMIO_03 := RATEIO->RAT_RATEIO
							case RATEIO->RAT_FAIXA == '04'
								pQNA_ACERTO_04 := RATEIO->RAT_ACERTA
								pQNA_PREMIO_04 := RATEIO->RAT_RATEIO
							case RATEIO->RAT_FAIXA == '05'
								pQNA_ACERTO_05 := RATEIO->RAT_ACERTA
								pQNA_PREMIO_05 := RATEIO->RAT_RATEIO	
						endcase
						RATEIO->( dbSkip() )
					enddo
				EndIf
				...
				while lContinua
					...
					@ oWindow:nTop+ 3, oWindow:nLeft+14 GET   pQNA_DEZENA_01                                   ;
														VALID .not. Empty( pQNA_DEZENA_01 )                    ;
														SEND  VarPut(StrZero(Val(ATail(GetList):VarGet()),2))  ;
														PICT  '@K 99'                                          ;
														COLOR SysFieldGet()

					@ oWindow:nTop+ 3, oWindow:nLeft+18 GET   pQNA_DEZENA_02                                   ;
														VALID .not. Empty( pQNA_DEZENA_02 )                    ;
														SEND  VarPut(StrZero(Val(ATail(GetList):VarGet()),2))  ;
														PICT  '@K 99'                                          ;
														COLOR SysFieldGet()

					@ oWindow:nTop+ 3, oWindow:nLeft+22 GET   pQNA_DEZENA_03                                   ;
														VALID .not. Empty( pQNA_DEZENA_03 )                    ;
														SEND  VarPut(StrZero(Val(ATail(GetList):VarGet()),2))  ;
														PICT  '@K 99'                                          ;
														COLOR SysFieldGet()

					@ oWindow:nTop+ 3, oWindow:nLeft+26 GET   pQNA_DEZENA_04                                   ;
														VALID .not. Empty( pQNA_DEZENA_04 )                    ;
														SEND  VarPut(StrZero(Val(ATail(GetList):VarGet()),2))  ;
														PICT  '@K 99'                                          ;
														COLOR SysFieldGet()

					@ oWindow:nTop+ 3, oWindow:nLeft+30 GET   pQNA_DEZENA_05                                   ;
														VALID .not. Empty( pQNA_DEZENA_05 )                    ;
														SEND  VarPut(StrZero(Val(ATail(GetList):VarGet()),2))  ;
														PICT  '@K 99'                                          ;
														COLOR SysFieldGet()
...
*	QNAExcluir()
*
*	Realiza a exclusao do concurso da QUINA.
*
*   QNAMntBrowse -> QNAExcluir
*
*/
STATIC PROCEDURE QNAExcluir
...
*	QNAGravaDados()
*
*	Realiza a gravacao dos dados da QUINA.
*
*/
STATIC FUNCTION QNAGravaDados
...
local lRetValue := pFALSE
	...
	begin sequence
		...
		while .not. lRetValue			
			...
			If iif( CONCURSO->( dbSetOrder(1), dbSeek( pQUINA + pQNA_CONCURSO ) ), CONCURSO->( NetRLock() ), CONCURSO->( NetAppend() ) )
				CONCURSO->CON_JOGO   := pQUINA
				CONCURSO->CON_CONCUR := pQNA_CONCURSO
				CONCURSO->CON_SORTEI := pQNA_SORTEIO
				CONCURSO->( dbUnlock() )
			EndIf

			If iif( JOGOS->( dbSetOrder(1), dbSeek( pQUINA + pQNA_CONCURSO ) ), JOGOS->( NetRLock() ), JOGOS->( NetAppend() ) )
				JOGOS->JOG_JOGO   := pQUINA
				JOGOS->JOG_CONCUR := pQNA_CONCURSO
				JOGOS->JOG_DEZENA := ParseString( aQuina[ pQNA_POS_DADOS ][ pQNA_POS_DEZENAS ] )
				JOGOS->( dbUnlock() )
			EndIf

			If iif( RATEIO->( dbSetOrder(1), dbSeek( pQUINA + pQNA_CONCURSO + '02' ) ), RATEIO->( NetRLock() ), RATEIO->( NetAppend() ) )
				RATEIO->RAT_JOGO   := pQUINA
				RATEIO->RAT_CONCUR := pQNA_CONCURSO
				RATEIO->RAT_FAIXA  := '02'
				RATEIO->RAT_ACERTA := pQNA_ACERTO_02
				RATEIO->RAT_RATEIO := pQNA_PREMIO_02
				RATEIO->( dbUnlock() )
			EndIf

			If iif( RATEIO->( dbSetOrder(1), dbSeek( pQUINA + pQNA_CONCURSO + '03' ) ), RATEIO->( NetRLock() ), RATEIO->( NetAppend() ) )
				RATEIO->RAT_JOGO   := pQUINA
				RATEIO->RAT_CONCUR := pQNA_CONCURSO
				RATEIO->RAT_FAIXA  := '03'
				RATEIO->RAT_ACERTA := pQNA_ACERTO_03
				RATEIO->RAT_RATEIO := pQNA_PREMIO_03
				RATEIO->( dbUnlock() )
			EndIf

			If iif( RATEIO->( dbSetOrder(1), dbSeek( pQUINA + pQNA_CONCURSO + '04' ) ), RATEIO->( NetRLock() ), RATEIO->( NetAppend() ) )
				RATEIO->RAT_JOGO   := pQUINA
				RATEIO->RAT_CONCUR := pQNA_CONCURSO
				RATEIO->RAT_FAIXA  := '04'
				RATEIO->RAT_ACERTA := pQNA_ACERTO_04
				RATEIO->RAT_RATEIO := pQNA_PREMIO_04
				RATEIO->( dbUnlock() )
			EndIf

			If iif( RATEIO->( dbSetOrder(1), dbSeek( pQUINA + pQNA_CONCURSO + '05' ) ), RATEIO->( NetRLock() ), RATEIO->( NetAppend() ) )
				RATEIO->RAT_JOGO   := pQUINA
				RATEIO->RAT_CONCUR := pQNA_CONCURSO
				RATEIO->RAT_FAIXA  := '05'
				RATEIO->RAT_ACERTA := pQNA_ACERTO_05
				RATEIO->RAT_RATEIO := pQNA_PREMIO_05
				RATEIO->( dbUnlock() )
			EndIf			
...
*
*   QNAMntBrowse -> QNAAcoes
*
*/
STATIC PROCEDURE QNAAcoes
...
*
*   QNAMntBrowse -> QNAAcoes -> QNACombina
*
*/
STATIC PROCEDURE QNACombina
...
local nOpcao      := 1
local nQuantJog   := 1
local nQuantDez   := pQNA_DEF_MIN_DEZENAS
local nQuantGrp   := 1
		...
		while lContinua
			...
			@ oWindow:nBottom- 1, oWindow:nRight-22 GET     lPushButton PUSHBUTTON                  ;
													...
													WHEN    nQuantJog >= 1 .and.                    ;
															nQuantJog <= pQNA_DEF_MAX_COMB .and.    ;
															nQuantDez >= pQNA_DEF_MIN_DEZENAS .and. ;
															nQuantDez <= pQNA_DEF_MAX_DEZENAS .and. ;
															nQuantGrp >= 1                          ;
													STATE   { || lContinua := pTRUE, GetActive():ExitState := GE_WRITE }
			...
			If lContinua .and. LastKey() != K_ESC
				...
				do case
					case nOpcao == 1
						QNAAnaliAleatoria( nQuantJog, nQuantDez )
					...
					case nOpcao == 4
						QNAGeraGrupos( nQuantJog, nQuantDez, nQuantGrp )
...
*	QNAAnaliAleatoria()
*
*	Realiza a geracao das dezenas para a QUINA aleatoriamente.
*
*   QNAMntBrowse -> QNAAcoes -> QNACombina -> QNAAnaliAleatoria
*
*   nQuantJogos : Informa a quantidade de jogos a ser geradas
* nQuantDezenas : Informa o numero de dezenas a ser geradas por jogos
*
*/
STATIC PROCEDURE QNAAnaliAleatoria( nQuantJogos, nQuantDezenas )
...
local aSequencia
	...
	DEFAULT nQuantJogos   TO 1, ;
			nQuantDezenas TO pQNA_DEF_MIN_DEZENAS
...
*	Realiza a geracao das dezenas para a QUINA aleatoriamente.
*
*   QNAMntBrowse -> QNAAcoes -> QNACombina -> QNAGeraGrupos
*
*   nQuantJogos : Informa a quantidade de jogos a ser geradas
* nQuantDezenas : Informa o numero de dezenas a ser geradas por jogos
...
*
*/
STATIC PROCEDURE QNAGeraGrupos( nQuantJogos, nQuantDezenas, nQuantGrupos )
...
*
*   QNAMntBrowse -> QNAAcoes -> QNACombina -> QNAAnaliAleatoria -> QNAShowAposta
*
*/
STATIC PROCEDURE QNAShowAposta
...
*	QNARelResult()
*
*	Realiza a impressao dos resultados da QUINA.
*
*   QNAMntBrowse -> QNAAcoes -> QNACombina -> QNARelResult
*
*/
STATIC PROCEDURE QNARelResult
...
local bFiltro      := { || CONCURSO->CON_JOGO == pQUINA .and. CONCURSO->( .not. Eof() ) }
local oBarProgress
local oPDFReport
local nLinha
		...
		if nTotConcurso >= 1
			...
			WHILE lContinua
				...
				IF lContinua .and. LastKey() != K_ESC
					...
					dbSelectArea('CONCURSO')
					IF CONCURSO->( dbSetOrder(1), dbSeek( pQUINA + cInicio ) ) .and. ;
						CONCURSO->( dbSetOrder(1), dbSeek( pQUINA + cFinal ) )

						bFiltro := { || CONCURSO->CON_JOGO == pQUINA .and. CONCURSO->( .not. Eof() ) .and. ;
						...
						begin sequence
							...
							oPDFReport:SetInfo( 'EDILSON MENDES', 'ODIN', 'RESULTADOS QUINA', oPDFReport:cFileName )

							nLinha := oPDFReport:MaxCol()

							//Posiciona o Registro
							If CONCURSO->( dbSetOrder(1), dbSeek( pQUINA + cInicio ) )
								...
								while Eval( bFiltro )
									...
									If nLinha >= ( oPDFReport:MaxCol() - 35 )
										...
										oPDFReport:DrawText( 1, 26, PadC( 'RESULTADOS QUINA', 70 ), , 10, 'Helvetica-Bold' )
